...
class LaporanPendapatanControllers extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // // **Mengelompokkan data berdasarkan tanggal, id_barang, kode_barang, merek, dan harga**

        // $groupedData = [];
        // foreach ($penjualan as $data) {
        //     $key = $data->tanggal . '-' . $data->id_barang . '-' . $data->kode_barang . '-' . $data->merek . '-' . $data->harga;

        //     // dd($key);


        //     if (!isset($groupedData[$key])) {
        //         $groupedData[$key] = (array) $data;
        //         $groupedData[$key]['total_pembelian_detail_barang_sama'] =
        //             ($data->harga - $data->diskon_barang) * $data->jumlah;
        //         $groupedData[$key]['total_jumlah'] = $data->jumlah;
        //     } else {
        //         $groupedData[$key]['total_pembelian_detail_barang_sama'] +=
        //             ($data->harga - $data->diskon_barang) * $data->jumlah;
        //         $groupedData[$key]['total_jumlah'] += $data->jumlah;
        ...
        // // // **Konversi kembali ke array indeks numerik**
        // $penjualanFinal = array_values($groupedData);
        ...
        //             'total_pembelian_detail_barang_sama' => $group->sum(function ($item) {
        //                 return ($item->harga - $item->diskon_barang) * $item->jumlah;
        //             }),
        //             'total_jumlah' => $group->sum('jumlah')
        //         ];
        //     })
        // ->values(); // Menghapus key agar array tetap indeks numerik
        ...
        //         // Cari harga pembelian terakhir berdasarkan id terbesar
        //         $hargaPembelianTerakhir = PembelianDetail::query()
        //             ->join('pembelian', 'pembelian_detail.id_pembelian', '=', 'pembelian.id')
        //             ->where('pembelian.tanggal', $firstItem->tanggal) // Tanggal harus sama dengan penjualan
        //             ->where('pembelian.id_lokasi', $firstItem->id_lokasi) // Lokasi sama
        //             ->where('pembelian_detail.id_barang', $firstItem->id_barang) // Barang sama
        //             ->where('pembelian_detail.merek', $firstItem->merek) // Merek sama
        //             ->where('pembelian_detail.harga', '<=', $firstItem->harga) // Harga <= harga penjualan
        //             ->orderBy('pembelian.id', 'desc') // Urutkan dari pembelian terakhir dibuat
        //             ->value('pembelian_detail.harga'); // Ambil harga pembelian terakhir
        ...
        //             }),
        //             'total_jumlah' => $group->sum('jumlah'),
        //             'harga_pembelian' => $hargaPembelianTerakhir ?? 0, // Simpan harga pembelian terakhir
        //         ];
        //     })
        // ->values(); // Menghapus key agar array tetap indeks numerik
